...
const IndivisualSplitExpense:React.FC<IndivisualExpenseSplitProps> = ({groupId, expenseTitle, totalExpense, paidByUserId,handleClose, triggerRefresh, resetForm}) => {
  ...
  setUsers((prev) =>
    prev.map((user) =>
      user._id === userId ? { ...user, isIncluded: checked,totalContri: 0 } : user
    )
  );
};

const validateTotal=()=>{
    const calculatedTotal  = users.filter(u=>u.isIncluded).reduce((sum, u)=>(sum + u.totalContri),0);
...
const handleSubmit = () => {
    ...
    const splitTax = (totalTax>0 && filteredUser.length>0 ) ? Math.round(totalTax/filteredUser.length) : 0;
  ...
  const splitTax = (totalTax > 0 && includedUsers.length > 0)
  ? Math.round(totalTax / includedUsers.length)
  : 0;

  return (
    <Container>
        <TextField label="Taxes" sx={{marginTop:2}} required value={totalTax} onChange={(e)=>setTotalTax(Number(e.target.value))}/>
        {users.map((user) => (
        ...
        {alertText && (<Typography sx={{marginBottom:2}}color="error"> {alertText}</Typography>)}
        <Grid container spacing={2}>
                <Grid size={{xs:12,sm:6}}>
                    {/* <Button fullWidth variant="contained" color="primary" onClick={handleSubmit}> */}
                    <Button fullWidth variant="contained" color="primary" onClick={validateTotal}>
                        Add Expense
                    </Button>
                </Grid>
                <Grid size={{xs:12,sm:6}}>
